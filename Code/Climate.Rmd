...
```{r setup, echo=F}
library(knitr)
opts_chunk$set(cache=T, autodep=T, root="../", warning=F, message=F)
```
...
```{r data-setup, echo=T, dependson='workspace'}
## Data Setup
...
fixClimateVar <- function(climateData, var="temp", projected=NULL, 
...
# Graphs
## Figure 1
```{r graph1, include=T, fig.width=8, fig.height=6, out.width="90%"}
...
monthRainDev <- monthRain %>%
  filter(Year==2015) %>%
  mutate(PctDevCumRain = paste0(sprintf("%.2f", (mean(CumRain[Year==2015])-AvgCumRain)/AvgCumRain*100), "%"))
